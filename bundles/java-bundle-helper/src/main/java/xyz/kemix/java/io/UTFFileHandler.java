...
public class UTFFileHandler {
	...
	public static UnicodeInputStream getInputStreamWithoutBom(InputStream is) throws IOException {

		UnicodeInputStream stream = null;
		try {
			stream = new UnicodeInputStream(is, CharsetNames.UTF_8);
			stream.getEncoding(); // try to remove the BOM
		} catch (Exception e) {
			throw new IOException("IO Stream Error!");
		}
		return stream;
	}
	...
	public static class UnicodeInputStream extends InputStream {
		...
		protected void init() throws IOException {
			...
			if ((bom[0] == (byte) 0x00) && (bom[1] == (byte) 0x00) && (bom[2] == (byte) 0xFE)
				...
				encoding = CharsetNames.UTF_16BE;
				unread = n - 2;
			} else if ((bom[0] == (byte) 0xFF) && (bom[1] == (byte) 0xFE)) {
				encoding = CharsetNames.UTF_16LE;
				unread = n - 2;
			} else {
